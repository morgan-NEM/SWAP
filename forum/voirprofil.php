...
switch($action)
{
    ...
    case "consulter":
       ...
       echo'<strong>Profil Facebook : </strong>'.stripslashes(htmlspecialchars($data['membre_facebook'])).'<br />';
...
case "modifier":
    ...
    else //Sinon on est dans la page de traitement
    {
        ...
        //Vérification de l’adresse Facebook
        if (!preg_match("#^[a-z0-9._-]+@[a-z0-9._-]{2,}\.[a-z]{2,4}$#", $facebook) && !empty($facebook))
        {
            $facebook_erreur = "Votre nouvelle adresse de profil Facebook n'a pas un format valide";
            $i++;
        }
